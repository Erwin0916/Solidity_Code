...
contract Vote{
    ...
    receive() external payable {  //플레이어로 부터 eth를 전송받기 위한 함수
        require(msg.value == requiredValue, string(abi.encodePacked("Must be ", valuestr, " ether.")));      
        //플레이어로 부터 전송받는 금액이 배팅금액과 동일 할때만 통과
        ...
        //모든 조건을 통과했다면 금액을 전송한 플레이어를 순서에 맞게게 paidAddressList리스트에 true상태로 저장
    ...
    function vote(uint256 choice) public payable {  //eth를 지불한 플레이어가 투표를 할 수 있도록 하는 함수
        ...
        require(Voted[round][msg.sender] == false && paidAddressList[round][msg.sender] == true , "Already voted or Did not send 0.01 ether yet");  
        // 이미 투표했거나 아직 eth를 전송하지 않은 플레이어 제한
